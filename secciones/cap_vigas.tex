...
\footnotetext{Una demostración de que la suma directa de espacios de Hilbert es también un espacio de Hilbert puede ser hallada en \textcolor{green}{CONWAY: A COURSE IN FUNCTIONAL ANALYSIS. CAP 1.6}}
...
con producto interno $(\cdot, \cdot)_{\mathcal{H}} : \mathcal{H}\times \mathcal{H} \rightarrow \mathbb{C}$ y norma $\lVert \, \cdot \, \rVert_{\mathcal{H}} : \mathcal{H} \rightarrow \mathbb{R}$ definidos, respectivamente, como:
...
\begin{Proposicion}{(Norma asociada al funcional de energía)}
    ...
    \begin{align}
        \label{normaEnergiaH}
        \lVert \, U \, \rVert^2_{E} = \rho_1 \lVert \varphi_t \rVert^2_{L^2} + \rho_2 \lVert \psi \rVert_{L^2}^2 + b \lVert \psi \rVert_{H_0^1}^2 + k \lVert \varphi_x + \psi \rVert_{L^2}^2 = 2E(U)
    \end{align}
